...
class RelatedTagUsageListData(BaseModel):
    """
    Complete non-zero usage result for the requested candidate tag ids.
    """ # noqa: E501
    items: List[RelatedTagUsageItem] = Field(description="One entry per requested tag id that is actually in use, sorted by id.")
    total: Annotated[int, Field(strict=True, ge=0)] = Field(description="Number of returned distinct tag ids")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "items": [RelatedTagUsageItem.from_dict(_item) for _item in obj["items"]] if obj.get("items") is not None else None,
            "total": obj.get("total")
        })
