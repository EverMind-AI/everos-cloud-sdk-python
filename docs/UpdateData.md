...
The edited record itself — the same shape ``SearchEpisodeItem`` uses for ``score``, not a wrapper around a nested ``item`` that would restate its ids.
...
**unchanged** | **bool** | True when the patch equalled what was already stored — nothing was written and nothing downstream was triggered. | [optional] [default to False]

## Example

```python
from everos_cloud.models.update_data import UpdateData
